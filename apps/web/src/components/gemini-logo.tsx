...
function getWordmarkWidth(height: number, cropTop = 0, cropLeft = 0, cropRight = 0) {
  const visibleHeight = GEMINI_WORDMARK_HEIGHT - cropTop;
  const visibleWidth = GEMINI_WORDMARK_WIDTH - cropLeft - cropRight;
  return Math.round((height * visibleWidth) / visibleHeight);
}

export function getGeminiLogoSvg({
  size = 24,
  gradientId = 'gemini-logo-gradient',
}: Pick<GeminiLogoProps, 'size' | 'gradientId'> = {}) {
  return `
    <svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" aria-hidden="true" xmlns="http://www.w3.org/2000/svg">
      <defs>
        <linearGradient id="${gradientId}" x1="4" y1="3" x2="19" y2="21" gradientUnits="userSpaceOnUse">
          ...
          <stop offset="0.52" stop-color="#5d8cff" />
          <stop offset="1" stop-color="#7a61ff" />
        </linearGradient>
      </defs>
      <path
        d="M12 2.25C13.04 6.88 14.87 8.71 19.5 9.75C14.87 10.79 13.04 12.62 12 17.25C10.96 12.62 9.13 10.79 4.5 9.75C9.13 8.71 10.96 6.88 12 2.25Z"
        fill="url(#${gradientId})"
      />
    </svg>
  `.trim();
...
export function getGeminiWordmarkSvg({
  height = 46,
  width = getWordmarkWidth(height),
  color = 'currentColor',
}: Pick<GeminiLogoProps, 'width' | 'height' | 'color'> = {}) {
  return `
    <svg
      width="${width}"
      height="${height}"
...
export function GeminiLogo({
  ...
  if (variant === 'wordmark') {
    ...
    const resolvedWidth = width ?? getWordmarkWidth(resolvedHeight, cropTop, cropLeft, cropRight);
    const visibleHeight = Math.max(1, GEMINI_WORDMARK_HEIGHT - cropTop);
    const visibleWidth = Math.max(1, GEMINI_WORDMARK_WIDTH - cropLeft - cropRight);

    return (
      <svg
        width={resolvedWidth}
        height={resolvedHeight}
        viewBox={`${cropLeft} ${cropTop} ${visibleWidth} ${visibleHeight}`}
        fill={color}
        fillRule="evenodd"
        aria-hidden="true"
        className={className}
    ...
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" aria-hidden="true" className={className}>
      <defs>
        <linearGradient id={gradientId} x1="4" y1="3" x2="19" y2="21" gradientUnits="userSpaceOnUse">
          ...
          <stop offset="0.52" stopColor="#5d8cff" />
          <stop offset="1" stopColor="#7a61ff" />
        </linearGradient>
      </defs>
      <path
        d="M12 2.25C13.04 6.88 14.87 8.71 19.5 9.75C14.87 10.79 13.04 12.62 12 17.25C10.96 12.62 9.13 10.79 4.5 9.75C9.13 8.71 10.96 6.88 12 2.25Z"
